...
- If your standard validation routine cannot address the following inputs, then they should be checked discretely:
  ...
  - Check for "dot-dot-slash" (`../` or `..\`) path alteration characters. In cases where UTF-8 extended character set encoding is supported, address alternate representation like: `%c0%ae%c0%ae/` (Utilize canonicalization to address double encoding or other forms of obfuscation attacks)
